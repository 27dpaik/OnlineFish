...
class OcLobbyScreen extends StatelessWidget {
  const OcLobbyScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Consumer<OcController>(
      builder: (context, ctrl, _) {
        ...
        if (s.phase == OcPhase.playing || s.phase == OcPhase.finished) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (_) => ChangeNotifierProvider.value(
                  value: ctrl,
                  child: const OcGameScreen(),
                ),
              ),
            );
          });
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text('One Card lobby — ${s.gameId}'),
            backgroundColor: const Color(0xFF1F2937),
